...
DATAsp2020=read_excel("C:/Users/happy/Documents/Github/patient.xlsx",col_names=as.character(1:21))
dim(DATAsp2020)        #[1] 224  21
head(DATAsp2020)       
sum(is.na(DATAsp2020)) #[1] 0: no missing data

# Adjust the data
ShockData=DATAsp2020[seq(1,224,2),1:20] #21 column shows record1 or record2
ShockData=as.data.frame(ShockData)
dim(ShockData)  #[1] 112  20, record1 remains and record2 removes
...
#categorical variables convert to 0,1
...
# Summary statistics
Sn=c() # number of patients
Smin=c()
Smax=c()
Smean=c()
Sstd=c()
for (i in 2:ncol(ShockData)) #column1=ID, column2=age
{
...
stepAIC(fit.all)    #Step:  AIC=119.09, Survive ~ Shock_TYP + MAP + MCVP + BSI + UO + RCI
...
aggregate(formula = Survive ~ HT+Shock_TYP+ MAP + MCVP + BSI, data = ShockData[, 2:20], FUN=mean)
